...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg,
  ...
  sprintf(body,
          ...
          ">\r\n",
          body); 
  sprintf(body, "%s%s: %s\r\n", body, errnum, shortmsg); // 버퍼에 형식 문자열을 대체해서 저장
  sprintf(body, "%s<p>%s: %s\r\n", body, longmsg, cause); // 상동
  sprintf(body, "%s<hr><em>The Tiny Web Server</em>\r\n", body); // 상동

  /* HTTP 응답 헤더 만들기 */
  sprintf(buf, "HTTP/1.0 %s %s\r\n", errnum, shortmsg); // 상동: buf에  HTTP/1.0 에러코드 에러메세지 쓰기
  Rio_writen(fd, buf, strlen(buf)); // 버퍼에서 strlen(buf)의 길이만큼 꺼내서 식별자가 가리키는 파일(fd)에 씀(참고로 버퍼에는 어떤 영향도 가지 않음)
